...
(defun evg-patch-title (patch)
  (if-let ((patch-number (evg-patch-number patch))
           (description (truncate-string-to-width (evg-patch-description patch) 50 nil nil t)))
      (format "Patch %d - %S" (evg-patch-number patch) description)
    (format "Revision %s - %S" (substring (evg-patch-revision patch) 0 7) description)))
  ...
  (if (eq task-format 'text)
      (let* ((not-passed-tasks (seq-filter (lambda (task)
                                             (let ((task-status (evg-task-info-status task)))
                                               (not (or (evg-status-passed-p task-status)
                                                        (evg-status-unscheduled-p task-status)))))
                                           tasks))
             (filter-tasks (lambda (status-fn)
                             (seq-filter
                              (lambda (task) (funcall status-fn (evg-task-info-status task)))
                              not-passed-tasks)))
             ...
             (other-tasks (cl-set-difference not-passed-tasks
                                             (append failed-tasks system-failed-tasks known-issue-tasks)))
...
results (either 'text or 'grid) and a previous buffer that can be returned to."
  (switch-to-buffer
   (get-buffer-create (format "evg-view-patch: %s" (evg-patch-title patch))))
  (read-only-mode -1)
     ...
     (insert
      (with-temp-buffer
        ...
        (let* ((tasks (cdr variant-tasks))
               (count-tasks (lambda (status-fn)
                              (seq-count (lambda (task) (funcall status-fn (evg-task-info-status task))) tasks)))
          ...
          (when (> n-system-failed 0)
            (setq aggregate-stats (cons (propertize (format "%s system failed" n-system-failed)
                                                    'face 'evg-status-text-system-failed)
                                        aggregate-stats)))
          (when (> n-known-issue 0)
            (setq aggregate-stats (cons (propertize (format "%s known issue" n-known-issue)
                                                    'face 'shadow)
                                        aggregate-stats)))
          (when (> n-started 0)
            (setq aggregate-stats (cons (propertize (format "%s started" n-started)
                                                    'face 'warning)
                                        aggregate-stats)))
          (when (> n-undispatched 0)
            (setq aggregate-stats (cons (propertize (format "%s undispatched" n-undispatched)
                                                    'face 'shadow)
                                        aggregate-stats)))
          (when (> n-unscheduled 0)
            (setq aggregate-stats (cons (propertize (format "%s unscheduled" n-unscheduled)
                                                    'face 'shadow)
                                        aggregate-stats)))
